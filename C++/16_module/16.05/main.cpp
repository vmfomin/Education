...
// По скольку тип enum один, делать его enum class не буду. Пересечения
// перечислений в одном файле не будет. А делать статик_каст утомительно.
enum Panel {
  POWER = 1 << 0,
  OUTLETS = 1 << 1,
  LIGHT_INSIDE = 1 << 2,
...
int main() {
  ...
  for (int16_t timeSimulation{}; timeSimulation < 48; ++timeSimulation) {
    ...
    cout << "Enter status:\x1b[32m\t";
    std::string textStatus{};
    std::getline(cin, textStatus);
    cout << "\x1b[0m";
  ...
  cout << "\x1b[36mThe simulation is complete. All sensors are off.\nYou can "
